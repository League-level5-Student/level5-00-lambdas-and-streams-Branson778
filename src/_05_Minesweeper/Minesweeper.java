...
public class Minesweeper extends PApplet {
     ...
     */
    boolean checkWin() {
       List<Cell> sort = cells;
    	long compare = 0;
       long count = sort.stream().filter((word) -> word.revealed == true && word.mine==false).count();
    	for(Cell cell: cells) {
    		if(cell.mine==false && cell.revealed==true) {
    			compare++;
    		}
    	}
    	if(compare==count) {
    ...
    void revealCell(Cell cell) {
        if(cell.mine==false && cell.revealed==false) {
        	...
        	if(cell.minesAround==0) {
        		...
        		neighbors.stream().forEach((celld) -> revealCell(celld));
        	}
   ...
   cells.stream().forEach((cell) -> cell.minesAround = getNeighbors(cell).stream().mapToInt((cell2) -> {
    	 if(cell2.mine)
    	 {return 1;} 
    	 else
    	 {return 0;}}
